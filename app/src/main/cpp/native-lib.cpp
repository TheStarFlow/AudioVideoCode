...
Java_com_example_yuvopengldemo_YuvPlayer_loadYuv(JNIEnv
                                                 *env,
...
//3.1 surface配置，可以理解为窗口
    ...
    if (EGL_TRUE !=
        eglChooseConfig(display, configSpec, &eglConfig,
                        1, &configNum)) {
...
//加入纹理坐标数据
    ...
    glVertexAttribPointer(aTex,
                          2, GL_FLOAT, GL_FALSE, 0, fragment);
...
//对sampler变量，使用函数glUniform1i和glUniform1iv进行设置
    glUniform1i(glGetUniformLocation(program, "yTexture"),
                0);
    glUniform1i(glGetUniformLocation(program, "uTexture"),
                1);
    glUniform1i(glGetUniformLocation(program, "vTexture"),
                2);
//纹理ID
    GLuint texts[3] = {0};
//创建若干个纹理对象，并且得到纹理ID
...
//设置纹理的格式和大小
    ...
    buf[0] = new unsigned char[
    width * height
    ];//y
    buf[1] = new unsigned char[
    width * height
    / 4];//u
    buf[2] = new unsigned char[
    width * height
    / 4];//v
...
//替换纹理，比重新使用glTexImage2D性能高
        glTexSubImage2D(GL_TEXTURE_2D,
                        0, 0, 0, width / 2, height / 2, GL_LUMINANCE,
                        GL_UNSIGNED_BYTE,
                        buf[1]);
